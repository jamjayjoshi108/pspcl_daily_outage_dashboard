...
#         st.markdown("### 📍 1. Zone-wise Distribution")
...
#                     selected_circle_yoy = yoy_circle.iloc[circle_selection.selection.rows[0]]["Circle"]

#                     if selected_circle_yoy != "Grand Total":
...
with tab2:
    ...
    if df_hist_curr.empty or df_hist_ly.empty:
        ...
        ytd_start = datetime(now_ist.year, 1, 1).date()
        ytd_end = now_ist.date()
        ly_ytd_start = safe_ly_date(ytd_start)
        ly_ytd_end = safe_ly_date(ytd_end)

        mask_ytd_curr = (df_hist_curr['Outage Date'] >= ytd_start) & (df_hist_curr['Outage Date'] <= ytd_end)
        mask_ytd_ly = (df_hist_ly['Outage Date'] >= ly_ytd_start) & (df_hist_ly['Outage Date'] <= ly_ytd_end)
